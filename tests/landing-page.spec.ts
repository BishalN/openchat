...
test.describe('Landing Page', () => {
    test('should display landing page with sign-in and key elements', async ({ page }) => {
        ...
        await expect(page.locator('a[href="https://github.com/bishaln/OpenChat"]').first()).toBeVisible();

        // Check for the logo and brand name in the header
        await expect(page.locator('header').getByText('OpenChat')).toBeVisible();
    ...
    test('should have working navigation links', async ({ page }) => {
        ...
        const githubLink = page.locator('a[href="https://github.com/bishaln/OpenChat"]').first();
    ...
    test('should display chat interface preview correctly', async ({ page }) => {
        ...
        await expect(page.getByText('Hi there! I\'m your OpenChat assistant')).toBeVisible();
        await expect(page.getByText('What makes OpenChat different from other AI platforms?')).toBeVisible();
